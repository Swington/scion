...
# --- Step: Push ---
# The VM pulls the repo directly in the remote build session below, so this local
# push only matters when you are iterating on hub source in THIS checkout and want
# the VM to build your unpushed changes. Deployments whose origin is a read-only
# upstream (e.g. a deployment overlay tracking GoogleCloudPlatform/scion) set
# SKIP_PUSH=true to skip it — the push would fail (no write access) and is moot
# because the VM pulls stock upstream anyway.

step "Pushing to origin..."
if [[ "${SKIP_PUSH}" == "true" ]]; then
    echo "  -> SKIP_PUSH=true; skipping local push (VM pulls upstream directly)."
elif [[ -n "$BRANCH" ]]; then
...
if $FULL_DEPLOY; then
    ...
    # settings.yaml — generated by emit_settings_yaml (hub-config.sh), which
    # folds in server.hub.admin_emails (HUB_ADMIN_EMAILS) and
    # telemetry.cloud.gcp_project_id (PROJECT_ID) when those are set.
    emit_settings_yaml > "$UPLOAD_DIR/scion-settings.yaml"
...
" "${REPO_DIR}" "${SCION_BIN}" "${BROKER_FLAGS}" > "$UPLOAD_DIR/scion-hub.service"
    ...
    # Caddyfile — Caddy terminates TLS on the VM with the Let's Encrypt cert and
    # reverse-proxies to the hub. In LB mode the load balancer re-encrypts to this
    # same Caddy endpoint on :443 ("double TLS").
    #
    # In LB mode the Google external HTTPS load balancer re-encrypts to Caddy on
    # :443 but does NOT forward SNI on that backend connection. Caddy's site block
    # is name-matched, so without a default SNI it has no certificate to serve for
    # the SNI-less ClientHello and aborts the handshake with a TLS internal error
    # — which the load balancer surfaces to clients as HTTP 502 (the :8080 health
    # check still passes because it bypasses this leg). The default_sni global
    # option makes Caddy present this site's certificate when SNI is absent.
    # Omitted for stock (non-LB) deployments, where Caddy terminates client TLS
    # directly and clients always send SNI, to keep the Caddyfile byte-identical.
    CADDY_GLOBAL=""
    if [[ "${ENABLE_LB}" == "true" ]]; then
        CADDY_GLOBAL="{
    default_sni ${HUB_DOMAIN}
}

"
    fi
    cat <<EOF > "$UPLOAD_DIR/Caddyfile"
${CADDY_GLOBAL}${HUB_DOMAIN} {
...
EOF
    ...
    gcloud compute scp ${SSH_TUNNEL_FLAG} "$UPLOAD_DIR"/* "${INSTANCE_NAME}:/tmp/" --zone="${ZONE}"
...
SUDOERS_EOF
    ...
    # Fix TLS certificate permissions for Caddy, then install the Caddyfile.
...
# shellcheck disable=SC2016,SC2086 # SC2016: remote $vars expand on the remote host; SC2086: SSH_TUNNEL_FLAG is intentionally word-split (flag or empty)
gcloud compute ssh "${INSTANCE_NAME}" --zone="${ZONE}" ${SSH_TUNNEL_FLAG} --command '
    set -euo pipefail
    RESET_DB='"${RESET_DB}"'
...
step "Remote health check..."
if [[ "${ENABLE_LB}" == "true" ]]; then
    echo "  -> LB mode: skipping external HTTPS health check here."
    echo "     The load balancer + Google-managed certificate are provisioned"
    echo "     separately by private-lb/lb.sh, which runs the end-to-end"
    echo "     https://${DOMAIN} check once the cert is ACTIVE."
    print_summary
    exit 0
fi
